		...
		switch (gunType) {
			case weaponType.pistol:
			    if (Time.frameCount - lastFired > fireRate){
				    GameObject bullet = (GameObject)Instantiate(bulletType, this.transform.position, Quaternion.LookRotation(-transform.parent.transform.forward, -Vector3.forward));
			...
			case weaponType.shotgun:
				...
				bullet = (GameObject) Instantiate(bulletType, this.transform.position, Quaternion.LookRotation(Quaternion.AngleAxis(-10, -Vector3.forward) * -transform.parent.transform.forward, -Vector3.forward));
				    bullet.GetComponent<Bullet>().damage = 4;
                    bullet.GetComponent<Bullet>().lifespan = 0.4f;
				bullet = (GameObject) Instantiate(bulletType, this.transform.position, Quaternion.LookRotation(Quaternion.AngleAxis(10, -Vector3.forward) * -transform.parent.transform.forward, -Vector3.forward));
				    bullet.GetComponent<Bullet>().damage = 4;
